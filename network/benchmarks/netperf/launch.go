...
//Gets the IP address of the primary Node
func getPrimaryNodeIP(c *kubernetes.Clientset) string {
        pods, err := c.Core().Pods("").List(
                metav1.ListOptions{
                        FieldSelector: "spec.nodeName=" + primaryNode.GetName(),
                })
        if err != nil {
                fmt.Println("Failed to fetch pods from secondary node", err)
                return ""
        }
...
func createRCs(c *kubernetes.Clientset) bool {
                                        ...
                                        Containers: []api.Container{
                                                {
                                                        ...
                                                        Env:             []api.EnvVar{{Name: "primaryNodeIP", Value: primaryNodeIP}},
        ...
        for i := 1; i <= 4; i++ {
                ...
                workerEnv := []api.EnvVar{
                        ...
                        {Name: "secondaryNodeIP", Value: primaryNodeIP},
                }
...
func main() {
        ...
        primaryNodeIP = getPrimaryNodeIP(c)
